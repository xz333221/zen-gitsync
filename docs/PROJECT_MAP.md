...
├── CHANGELOG.md             # 版本变更
├── 源码笔记.md               # 中文源码结构笔记
├── docs/migrations/MIGRATION_ICON_BUTTON.md  # 1.x 迁移到 2.x 的迁移指南
├── docs/migrations/SVG_ICON_SETUP.md  # SVG icon 组件接入文档
├── docs/                    # [文档目录]
...
│   └── verify/              # 验证截图产物
├── .agents/  # 其它 AI 工具主配置(.codebuddy/ .iflow/ .trae/ .windsurf/ 已删除本地副本,仅 .agents/ 仍在)
...
| 路径 | 说明 |
|------|------|
| `.agents/` | 项目级 AI 工具 skills/rules 镜像(.codebuddy/ .iflow/ .trae/ .windsurf/ 已删除本地副本,内容已迁移到 `.claude/`) |
