 ...
 *    内容几乎总为空):无法从锁文件本身确认持有者是否仍在运行,因此永不自动删除。
 *    mtime 只能用于诊断,不能证明锁已失效;长时间运行的 git 操作同样可能保持旧 mtime。
 *
 * 注:force 参数保留用于兼容现有调用方,但不会绕过并发保护。无法确认锁的
 *    持有者时宁可让 Git 返回 lock 错误,也不删除锁破坏 Git 的互斥机制。
 *
 * @param {Object} [options]
 * @param {boolean} [options.force=false] 兼容参数;当前不会绕过并发安全检查。
...
async function checkAndClearGitLock({ force = false } = {}) {
  try {
    ...
    if (isPidFormat) {
      ...
      // 原生 Git 的锁通常是空文件,无法从内容判断持有进程是否已退出。
      // mtime 不能证明锁已失效(长时间运行的 git 操作也可能不更新 mtime),
      // 因此包括 force=true 在内都不自动删除,让 Git 自己维持互斥保护。
      try {
        const stat = await fs.stat(lockFilePath);
        const ageSeconds = Math.max(0, Math.round((Date.now() - stat.mtimeMs) / 1000));
        console.log(chalk.yellow(`⚠️ 无法确认 Git 锁持有进程(年龄 ${ageSeconds}s),跳过自动清理`));
      } catch (_) {}
      return false;
    }
