...
/**
 * 右栏上限 900（原来 560）。560 在 1920 上只有屏幕的 29% ——
 * 而这一栏是"下指令 + 看活动流"的地方，指令常常十几行、活动流一行一条，
 * 用户的原话是"我主要就用这块来添加任务"，很容易撞到 560 就再也拖不动了。
 * 900 是"再宽就开始不像一条边栏"的经验值，具体能不能拖到由下面的视口比例再卡一道。
 */
const RIGHT_W_MAX = 900
/**
 * 单侧最多占视口的比例。
 * 右栏放到 46%（原来 36%）：上限 900 在小屏上会占掉大半个屏幕，得按比例收回来。
 * 左栏仍是 26% —— 项目名 + 任务数那点内容，再宽也是空着。
 * 两侧不会同时拉满（左栏默认 264），真同时拉满时看板会被压到 ~30%，
 * 那是用户自己拉出来的，双击任一分隔条即可退回默认值。
 */
const LEFT_W_SHARE = 0.26
const RIGHT_W_SHARE = 0.46
