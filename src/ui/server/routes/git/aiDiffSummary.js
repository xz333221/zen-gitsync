...
function buildPrompt({ scope, source, file, diffText, isUntracked, locale }) {
  ...
  if (scope === 'file') {
    const intro = isUntracked
      ? `文件 \`${file}\` 是一个新增(未跟踪)文件,以下是其完整内容(可能已截断):`
      : `以下是${where}中文件 \`${file}\` 的 git diff(可能已截断):`;
    return `${intro}
...
3. 如有潜在风险或值得注意的副作用,最后单独列出;没有就省略该部分。
保持简洁,总长度控制在 200 字以内。`;
  }
...
3. 如有潜在风险、破坏性变更或需要 reviewer 重点关注的地方,最后单独列出;没有就省略该部分。
保持简洁,总长度控制在 400 字以内。`;
}
 ...
 */
async function collectDiff({ execGitCommand, scope, source, file, hash }) {
  if (source === 'commit') {
  ...
  // worktree
  if (scope === 'overall') {
    const { stdout } = await execGitCommand(['diff', 'HEAD'], { log: false });
    return { diffText: stdout || '', isUntracked: false };
  }
  ...
  const guard = await ensureWithinCwd(String(file), cwd);
  if (!guard) throw new Error('禁止访问工作目录以外的文件');
  const content = await fs.readFile(guard.safePath, 'utf8');
  return { diffText: truncateDiff(content, MAX_UNTRACKED_CONTENT_CHARS), isUntracked: true };
}
...
export function registerAiDiffSummaryRoutes({ app, execGitCommand, configManager }) {
  app.post('/api/ai/diff-summary', express.json(), async (req, res) => {
    ...
    try {
      ...
      // 取 diff
      const { diffText, isUntracked } = await collectDiff({ execGitCommand, scope, source, file, hash });
      if (!diffText.trim()) {
      ...
      const prompt = buildPrompt({
        ...
        source,
        file,
        diffText: truncateDiff(diffText, budget),
        isUntracked,
        locale
      });

      const systemPrompt = '你是资深代码评审助手,擅长阅读 git diff 并用简洁准确的语言总结代码变更。只输出总结本身,不要复述指令。';
