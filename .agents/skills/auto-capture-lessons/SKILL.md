...
| 经验性质 | 目标位置 | 典型场景 |
|---------|---------|---------|
| **跨多个工作流/组件通用的工程经验** | 新建或追加到 `.Codex/skills/<name>/SKILL.md` | "preview 启好但浏览器走的是 backend 生产 bundle"、"vue-tsc 要 cd 到 client 目录" |
| **项目特定的事实/约定/陷阱** | `.Codex/rules/` 或项目内 `docs/` | "Job 状态字段是 running/pending/done 不是 active"、"pre 块最大 64KB 截断" |
| **用户偏好 / 协作风格 / AI 行为习惯** | `~/.Codex/projects/<proj>/memory/*.md` + `MEMORY.md` | "用户希望先看 computed style 再看 snapshot"、"用户偏好短响应不带总结" |
...
追加方式:用 Edit 在最相关的章节插入,优先放在"步骤 0a / 步骤 1"这种靠前位置(防止 Codex 看不到),或"不要做的事"表格末尾。

### 2b. 写入 rules

`.Codex/rules/` 是给本项目 Codex 协作规则用的,每次会话都加载。
...
- **feedback**:用户对 Codex 行为的具体纠正或确认(如"用户不喜欢响应末尾加总结")。
...
**判定**:对 Codex 行为的主观约束,跨所有任务。

**执行**:
- Write `~/.Codex/projects/<proj>/memory/feedback_no_trailing_summary.md`
- 追加一行到 `MEMORY.md`
...
**执行**:
- Edit `.Codex/rules/workbench-types.md`(如果存在)或新建小节
...
| 写了之后不验证 skill 真的会被加载 | 写到 `.Codex/skills/` 但 description 写得模糊 → Codex 不会触发 |
